    ...
    <section className={`py-16 ${dark ? "bg-[#080808]" : "bg-white"}`}>
      <div className="container">
        <div className="text-center mb-10">
          ...
          <h2 className={`text-3xl font-bold mb-2 ${dark ? "text-white" : "text-[#080808]"}`} style={{ fontFamily: 'Sora, sans-serif' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-4xl mx-auto">
          {testimonials.map((t, i) => (
            ...
            >
              <Quote size={24} className="text-[#c9a84c]/30 mb-3" />
              <p className={`text-sm leading-relaxed mb-4 ${dark ? "text-white/70" : "text-gray-600"}`}>
                "{t.text}"
              </p>
              <div className="flex items-center justify-between">
                <div>
                  <div className={`font-semibold text-sm ${dark ? "text-white" : "text-[#080808]"}`}>{t.name}</div>
