...
- [x] Fix 12: Improve internal linking — add Utah service area sidebar to blog posts for cross-linking
- [x] Fix 13: Distinguish H1 from title on utility/legal pages (AccessibilityStatement updated)
- [x] Fix 14: Optimize content on /industries/adult-entertainment, /industries/online-pharmacy, /industries/subscription-continuity (expanded to 300+ words each)
...
## Round 11 Fixes — Duplicate Title Tags (Jun 18, 2026)

- [x] R11-DupTitle: Fix 13 pages sharing the generic title "UBC Unlimited — Utah Merchant Services" — ROOT CAUSE: SPA serves same index.html for all routes; SEMrush reads static HTML before JS executes. FIXED by adding server-side title injection in vite.ts (both dev and prod catch-all routes). Created titleMap.ts with comprehensive route→title map covering all static routes, industry slugs, solution slugs, blog slugs, city slugs, and county slugs. The Express server now reads the URL path and injects the correct <title> tag before sending the HTML response.
- [x] R11-AboutContact: Updated About.tsx and Contact.tsx SEO title props to remove double brand mention ("About UBC Unlimited" → "About Us", "Contact UBC Unlimited" → "Contact Us") since the | UBC Unlimited suffix is appended automatically.
- [x] R11-FlakeyTest: Fixed flaky testimonials.test.ts timeout — root cause was missing mock for verifyRecaptcha (server/recaptcha.ts), causing real HTTP calls to Google's reCAPTCHA API during tests. Added vi.mock("./recaptcha") to prevent external calls. All 30 tests now pass consistently.

## Round 13 Fixes — Sitemap Issues (Jun 18, 2026)

- [x] R13-Sitemap: Rebuilt sitemap.xml completely — changed all 120 www URLs to non-www (https://ubcunlimited.com), replaced all /locations/ paths with correct /cities/ and /counties/ paths, added all 134 city pages under /cities/:slug, all 29 county pages under /counties/:slug, and missing static pages (/build-a-pos, /news, /accessibility, /legal/*). New sitemap has 236 URLs total. Zero /locations/ URLs remain. Zero www URLs remain.
- [x] R13-Orphaned: The orphaned pages issue was caused by the sitemap pointing to /locations/ paths that don't exist as real routes (the real routes are /cities/ and /counties/). By rebuilding the sitemap with correct /cities/ and /counties/ paths, all 134 city pages and 29 county pages now have valid sitemap entries. Internal links from blog posts to city/county pages were already added in Round 3 (b6108b8b checkpoint) — 107 city link instances and 63 county link instances across all 19 blog posts.

## Round 14 — llms.txt & robots.txt Fixes (Jun 18, 2026)
...
## Round 20 — Black Screen Fix (Production)
- [x] R20-BlackScreen: Fixed black screen on live site. Root cause: `vitePluginManusRuntime()` was included in production builds, injecting a 366 KB inline `<script id="manus-runtime">` (Manus visual editor runtime) into every production HTML page. This script ran synchronously before the module script and conflicted with React's `createRoot` call, causing `#root` to remain empty. Fix: gated `vitePluginManusRuntime()` to development mode only in `vite.config.ts` using `process.env.NODE_ENV !== 'production'`. Production build now produces clean HTML with no inline runtime script. All 30 tests pass.
